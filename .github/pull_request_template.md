...
- [ ] Env nova ou alterada → declarada em `apps/api-gateway/values.yaml` e no
...
Merge na `main` publica `ghcr.io/audita-bids/api-gateway:<sha>` e fixa a tag em
`apps/api-gateway/values-production.yaml`, sem aprovação manual. Merge na `staging`
faz o mesmo em `values-staging.yaml`. Rollback é reverter o commit
`ci(api-gateway): deploy …` no repo de infra.
